...
vi.mock('sonner', () => ({ toast: { success: vi.fn(), error: vi.fn() }, toast: { success: vi.fn(), error: vi.fn() } }));

// Mock Auth hook module for form tests
...
describe('Auth Forms', () => {
  it('calls onSuccess when login succeeds', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText(/•+/i), 'pwd');
...
// SnakeGame keyboard behavior tests
...
vi.mock('@/contexts/AuthContext', () => ({ useAuth: () => ({ user: { id: 'u1', username: 't' , highScore: 0 }, isLoading: false }) }));

import SnakeGame from '@/components/Game/SnakeGame';

describe('SnakeGame keyboard handling', () => {
  it('calls setDirection when modal is closed', async () => {
    const game = render(<SnakeGame isModalOpen={false} />);
  ...
  it('does not call setDirection when modal is open', async () => {
    ...
    fireEvent.keyDown(window, { key: 'w' });
    const gl = await import('@/lib/game-logic');
    expect(gl.setDirection).not.toHaveBeenCalled();
  });
});
